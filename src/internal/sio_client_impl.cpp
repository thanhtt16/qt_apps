...
client_impl::client_impl()
  ...
  m_client.set_message_handler(
      lib::bind(&client_impl::on_message, this, _1, _2));
#if SIO_TLS
  m_client.set_tls_init_handler(lib::bind(&client_impl::on_tls_init, this, _1));
  qDebug() << "set_tls_init_handler";
#endif
  m_packet_mgr.set_decode_callback(
      lib::bind(&client_impl::on_decode, this, _1));

  m_packet_mgr.set_encode_callback(
      lib::bind(&client_impl::on_encode, this, _1, _2));
}
...
void client_impl::connect(const string& uri, const map<string, string>& query,
  ...
  string query_str;
  for (map<string, string>::const_iterator it = query.begin();
       it != query.end(); ++it) {
    query_str.append("&");
  ...
  m_client.get_io_service().dispatch(
      lib::bind(&client_impl::connect_impl, this, uri, m_query_string));
  m_network_thread.reset(
      new thread(lib::bind(&client_impl::run_loop, this)));  // uri lifecycle?
}
...
socket::ptr const& client_impl::socket(string const& nsp) {
  ...
  if (it != m_sockets.end()) {
    ...
    pair<const string, socket::ptr> p(
        aux, shared_ptr<sio::socket>(new sio::socket(this, aux)));
...
void client_impl::close() {
  ...
  m_client.get_io_service().dispatch(lib::bind(
      &client_impl::close_impl, this, close::status::normal, "End by user"));
}
...
void client_impl::close_impl(close::status::value const& code,
                             string const& reason) {
...
void client_impl::send_impl(shared_ptr<const string> const& payload_ptr,
...
void client_impl::ping(const boost::system::error_code& ec) {
  if (ec || m_con.expired()) {
    if (ec != boost::asio::error::operation_aborted)
      LOG("ping exit,con is expired?" << m_con.expired()
                                      << ",ec:" << ec.message() << endl);
    return;
  }
  ...
  if (m_ping_timer) {
    ...
    m_ping_timer->async_wait(
        lib::bind(&client_impl::ping, this, lib::placeholders::_1));
  }
  if (!m_ping_timeout_timer) {
    m_ping_timeout_timer.reset(
        new boost::asio::deadline_timer(m_client.get_io_service()));
    boost::system::error_code timeout_ec;
    m_ping_timeout_timer->expires_from_now(milliseconds(m_ping_timeout),
                                           timeout_ec);
    m_ping_timeout_timer->async_wait(
        lib::bind(&client_impl::timeout_pong, this, lib::placeholders::_1));
  }
}
...
void client_impl::timeout_pong(const boost::system::error_code& ec) {
  ...
  m_client.get_io_service().dispatch(lib::bind(&client_impl::close_impl, this,
                                               close::status::policy_violation,
                                               "Pong timeout"));
}
...
void client_impl::timeout_reconnect(boost::system::error_code const& ec) {
  ...
  if (m_con_state == con_closed) {
    ...
    m_client.get_io_service().dispatch(lib::bind(
        &client_impl::connect_impl, this, m_base_url, m_query_string));
  }
}

unsigned client_impl::next_delay() const {
  // no jitter, fixed power root.
  unsigned reconn_made = min<unsigned>(
      m_reconn_made, 32);  // protect the pow result to be too big.
  return static_cast<unsigned>(
      min<double>(m_reconn_delay * pow(1.5, reconn_made), m_reconn_delay_max));
}
...
void client_impl::on_fail(connection_hdl con) {
  ...
  if (m_reconn_made < m_reconn_attempts) {
    ...
    m_reconn_timer->async_wait(lib::bind(&client_impl::timeout_reconnect, this,
                                         lib::placeholders::_1));
  } else {
...
void client_impl::on_close(connection_hdl con) {
  ...
  if (code == close::status::normal) {
    ...
    if (m_reconn_made < m_reconn_attempts) {
      ...
      boost::system::error_code ec;
      m_reconn_timer->expires_from_now(milliseconds(delay), ec);
      m_reconn_timer->async_wait(lib::bind(&client_impl::timeout_reconnect,
                                           this, lib::placeholders::_1));
      return;
    }
...
void client_impl::on_handshake(message::ptr const& message) {
  if (message && message->get_flag() == message::flag_object) {
    ...
    if (it != values->end() &&
        it->second->get_flag() == message::flag_integer) {
      m_ping_interval =
          (unsigned)static_pointer_cast<int_message>(it->second)->get_int();
    } else {
    ...
    m_ping_timer->async_wait(
        lib::bind(&client_impl::ping, this, lib::placeholders::_1));
    LOG("On handshake,sid:" << m_sid << ",ping interval:" << m_ping_interval
                            << ",ping timeout" << m_ping_timeout << endl);
    return;
  }
failed:
  // just close it.
...
void client_impl::on_encode(bool isBinary,
                            shared_ptr<const string> const& payload) {
  LOG("encoded payload length:" << payload->length() << endl);
  m_client.get_io_service().dispatch(
      lib::bind(&client_impl::send_impl, this, payload,
